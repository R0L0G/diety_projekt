...
def data_merge(file1 , file2):

    Copy_of_MyFoodData = pd.ExcelFile(file1)
    Copy_of_MyFoodData_DF = pd.read_excel(Copy_of_MyFoodData, "SR Legacy and FNDDS")

    Composition_of_Foods_Dataset = pd.ExcelFile(file2)
    ...
    return Composition_of_Foods_Dataset_DF


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Possible Arguments!!!",
                                     formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument("-my", "--MyFoodData")
    parser.add_argument("-mc", "--McCanceDataSet")
    args = vars(parser.parse_args())
    data_merge(args["MyFoodData"],args["McCanceDataSet"])
